...
export function Steps({ data, primaryColor }: Props) {
    return (
        <div className="my-2">
            {data.title && (
                <h3 className="text-sm font-semibold text-gray-900 mb-2">{data.title}</h3>
            )}
            {data.intro && (
                <p className="text-sm text-gray-700 mb-3 leading-relaxed">{data.intro}</p>
            )}
            ...
            {data.followUp && (
                <p className="text-sm text-gray-700 mt-3 leading-relaxed">{data.followUp}</p>
            )}
        </div>
    )
}